...
kind create cluster --name ${NETWORK_CLUSTER_NAME:-kind} --config scripts/kind-config.yaml

# Create the Kube namespace
kubectl create namespace ${NETWORK_NAMESPACE:-liberty-dlt}
...
```shell
export NETWORK_STAGE_DOCKER_IMAGES=true
...
export NETWORK_FABRIC_CONTAINER_REGISTRY=hyperledger-fabric.jfrog.io
export NETWORK_FABRIC_VERSION=amd64-latest
export NETWORK_FABRIC_CA_VERSION=amd64-latest
...
export NETWORK_FABRIC_VERSION=2.4.0
...
aliases (e.g. grpcs://org1-peer1.liberty-dlt.svc.cluster.local:443) and traverse private K8s network routes.
